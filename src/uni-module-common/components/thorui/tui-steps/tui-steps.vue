    ...
    >
      ...
      >
        <view
          ...
          :style="{
            ...
            backgroundColor:
              index <= activeSteps ? getActiveColor : type == 2 ? '#fff' : deactiveColor,
            borderColor: index <= activeSteps ? getActiveColor : deactiveColor
          }"
        >
          <text
            v-if="activeSteps !== index"
            :style="{ color: index <= activeSteps ? '#fff' : '' }"
            >{{ type == 1 ? '' : index + 1 }}</text
          >
          <tui-icon v-if="activeSteps === index" name="check" :size="16" color="#fff"></tui-icon>
        </view>
        <view v-if="item.name || item.icon" class="tui-step-custom" :style="{ backgroundColor }">
          <tui-icon
            v-if="item.name"
            :name="item.name"
            :size="item.size || 20"
        ...
        <view
          ...
          class="tui-step-line"
          :class="[
            `tui-step-${direction}_line`,
            direction == 'column' && (item.name || item.icon) ? 'tui-custom-left' : ''
          ]"
          :style="{
      ...
      <view class="tui-step-item-main" :class="[`tui-step-${direction}_item_main`]">
        <view style="display: flex; justify-content: flex-start">
          ...
          <view
            v-if="item.title.includes('已收货')"
            class="view-address"
            :style="{ lineHeight: `${titleSize}rpx` }"
            @tap="clickLogistics"
            >查看物流信息</view
          >
        </view>
        <view
          class="tui-step-item-content"
          :style="{
            color: index <= activeSteps ? getActiveColor : deactiveColor,
